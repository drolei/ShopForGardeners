       ...
       /* public static void Initial(AppDBContent content)
        {
            ...
            if (!content.AllCategories.Any())
            {
                content.AllCategories.AddRange(Categories.Select(c => c.Value));
            }

            if (!content.AllItems.Any())
            {
                content.AllItems.AddRange(items.Select(c => c.Value));
            }
            content.SaveChanges();
        }
            ...
            get
            {
                if(item == null)
                {
                    ...
                    new GardeningItem{Name="Gardener’s Pro Pruner #2",
                        ...
                        IsFavourite=true,
                        Available=true,
                        Category=Categories["Garden tool"]  },
                    ...
                    item = new Dictionary<string, GardeningItem>();

                    foreach (GardeningItem el in list)
                    {
                        item.Add(el.Name, el);
                    }
